...
export class UsuarioService
{
     ...
     * @param search
     */
    getUsuarios(page: number = 0, size: number = 10, sort: string = 'name', order: 'asc' | 'desc' | '' = 'asc', search: string = ''):
        Observable<{ pagination: InventoryPagination; usuarios: Usuario[] }>
    {
        return this._httpClient.get<{ pagination: InventoryPagination; usuarios: Usuario[] }>(this.apiUrl + 'usuario/users', {
            params: {
                page: '' + page,
                size: '' + size,
                sort,
            ...
            tap((response) => {
                ...
                this._pagination.next(response.pagination);
                this._usuarios.next(response.usuarios);
            })
        );
    }
    ...
    {        
        return this._httpClient.post<Usuario>(this.apiUrl + 'usuario/users', novoUsuario).pipe(
            map((novoUsuario) => {
                // Atualiza a lista de usuários com o novo usuário cadastrado
                this._usuarios.next([novoUsuario, ...this._usuarios.getValue()]);
